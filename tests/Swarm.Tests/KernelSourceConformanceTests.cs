...
public sealed class KernelSourceConformanceTests
{
    ...
    private static string[] PathIds()
    {
        ...
        var ids = abi.Select(StripComment)
                     ...
                     .ToArray();
        Assert.Contains("PATH_AUTO", ids);            // a renamed set must fail loudly
        Assert.Contains("PATH_AVX2", ids);
    ...
    /// said AVX-512 while the instructions were AVX2's. A bench row or a golden
    /// keyed on that word carried a path attribution the instructions did not
    /// support.
    ///
    /// Two properties, because either alone passes the shape that was in the
    /// tree. Containment alone - every resolvable id has an arm - was satisfied
    /// by the alias, since PATH_AVX512 had an arm; it just went to somebody
    /// else's body. Distinctness alone would pass a ladder resolving an id with
    /// no arm at all.
    ///
    /// WHAT THIS CANNOT SEE, stated because the scan reads stronger than it is.
    /// It works on tokens and jump targets, not on control flow: it cannot tell
    /// an init_core arm that ACCEPTS an id from one that REFUSES it, which is
    /// why PATH_AVX512's refusal is written as a fall-through naming nothing.
    /// It cannot tell two labels apart as bodies: two distinct targets count as
    /// distinct whatever they expand to. And it reads source rather than the
    /// assembled image, so it says nothing about what either routine encodes.
    /// </summary>
    [Fact]
