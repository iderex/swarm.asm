...
public sealed unsafe class ArenaTests
{
    ...
    // included, because no zmm body exists to run (#316). Before that, the id
    // was accepted wherever CPUID reported AVX-512 and the ymm body ran, so
    ...
    // assertion it makes is the same on both sides of the bit-1 divide, so the
    // host it runs on cannot decide whether the property is checked.
    //
    // WHAT MOVED OUT OF COVERAGE WITH IT. This replaces a test that requested
    // AVX-512 as "a path the machine lacks" and therefore observed the
    // CPU-feature gate on a host without the feature, skipping where the
    // feature was present. That gate is now DELETED for this id rather than
    // shadowed by the refusal below - init.inc names PATH_AVX512 in no arm at
    // all - so there is no second branch hiding behind a green here. What is
    // no longer exercised anywhere is a CPU-driven IERR_PATH: the arms that
    // still carry one gate PATH_AVX2 and PATH_AVX2_FMA on CPU_AVX2, which no
    // host that can load this kernel lacks. That branch is unreachable from
    // the suite rather than untested by choice, and it is stated here because
    // a reader counting fail-closed coverage would otherwise count it.
    [Fact]
    ...
    public void InitWritesHeader()
    {
        ...
        // ladder may only resolve to an id that has a body, and PATH_AVX512
        // has none (#316). This expectation used to branch on the CPU and read
        // 2 on a bit-1 host, which pinned the mislabelling as correct.
